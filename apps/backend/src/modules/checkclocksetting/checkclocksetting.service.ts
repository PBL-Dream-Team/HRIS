...
export class CheckClockSettingService {
  ...
  async createCheckClockSetting(dto: createCheckClockSettingDto) {
    try {
      const checkClockSetting = await this.prisma.checkClockSetting.create({
        data: dto,
      });
      return {
        statusCode: 201,
  ...
  async getCheckClockSetting(checkClockSettingId: string) {
    const data = await this.prisma.checkClockSetting.findFirst({
      where: { id: checkClockSettingId },
    });
    if (data) {
      return {
        statusCode: 200,
  ...
  async updateCheckClockSetting(
    checkClockSettingId: string,
    dto: editCheckClockSettingDto,
  ) {
    try {
      const checkClockSetting = await this.prisma.checkClockSetting.update({
        where: { id: checkClockSettingId },
        data: dto,
      });
      return {
  ...
  async deleteCheckClockSetting(checkClockSettingId: string) {
    try {
      await this.prisma.checkClockSetting.delete({
        where: { id: checkClockSettingId },
      });
      return {
        statusCode: 200,
