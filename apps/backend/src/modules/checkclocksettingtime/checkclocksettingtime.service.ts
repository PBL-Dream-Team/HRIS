...
export class CheckClockSettingTimeService {
  ...
  async createCheckClockSettingTime(dto: createCheckClockSettingTimeDto) {
    let data: any = { ...dto };
    try {
      const checkClockSettingTime =
        await this.prisma.checkClockSettingTime.create({ data: data });
      return {
        statusCode: 201,
  ...
  async getCheckClockSettingTime(checkClockSettingTimeId: string) {
    const data = await this.prisma.checkClockSettingTime.findFirst({
      where: { id: checkClockSettingTimeId },
    });
    if (data) {
      return {
        statusCode: 200,
  ...
  async updateCheckClockSettingTime(
    checkClockSettingTimeId: string,
    dto: editCheckClockSettingTimeDto,
  ) {
    try {
      const checkClockSettingTime =
        await this.prisma.checkClockSettingTime.update({
          where: { id: checkClockSettingTimeId },
          data: dto,
        });
      return {
  ...
  async deleteCheckClockSettingTime(checkClockSettingTimeId: string) {
    try {
      await this.prisma.checkClockSettingTime.delete({
        where: { id: checkClockSettingTimeId },
      });
      return {
        statusCode: 200,
